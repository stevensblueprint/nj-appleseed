export default function ManualComp({manual}) {
    return (
        <section className="px-6 py-12 max-w-6xl mx-auto">
            ...
            {manual.map(( chapter ) => (
                <input id={chapter.id} type="radio" name="manualtabs" title={`${chapter.id}-tab`} className={`hidden peer/${chapter.id}`} ng-class="${chapter.id==intro ? defaultChecked}" />
            ))}

            {/* Div holding all the labels */}
            <div className="flex flex-row items-end overflow-scroll">
            {manual.map(( chapter ) => (
            <label for={chapter.id} className="cursor-pointer pb-1">
